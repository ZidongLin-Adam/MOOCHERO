...
public class SplineTrailRenderer : MonoBehaviour 
{
    ...
    public void ChangeMaterial()
    {
        ...
        {
            currentMaterialIndex = ++currentMaterialIndex % myMaterials.Length;
            GetComponent<Renderer>().material = myMaterials[currentMaterialIndex];
            //Debug.Log(currentMaterialIndex);
        }
        RenderMesh();
    }

    //删除点
    public void RemoveKnots(int index, int count)
    {
        List<Knot> knots = spline.knots;
        transform.position = knots[index - 3].position;
        knots[index - 2].position = transform.position;
        knots[index - 1].position = transform.position;
        for (int i = spline.knots.Count - 1; i >= index; i--)
            knots.RemoveAt(i);
    ...
    //增加点
    public void AddKnots(Vector3 knotPosition)
    {
        spline.knots.Add(new Knot(knotPosition));
        transform.position = knotPosition;
        RenderMesh();
    }
